...
load("example/results/interval uncorrected/objects.Rdata")
...
ggsave("example/results/interval uncorrected/plots/parameter_uncertainty.pdf",
       p_pars, width = 12, height = 12)

p_phi_gamma <- plotScatterPhiGamma(df_pars, delta_t)
ggsave("example/results/interval uncorrected/plots/estphigamma.pdf",
       p_phi_gamma, width = 4, height = 3)

p_pmps <- plotPMPs(df_pmps)
ggsave("example/results/interval uncorrected/plots/posterior_bar.pdf",
       p_pmps, height = 4, width = 12)

p_prior_posterior <- plotPriorPosterior(df_posteriors, df_bayes_factors, delta_t)
ggsave("example/results/interval uncorrected/plots/bayesfactors.pdf",
