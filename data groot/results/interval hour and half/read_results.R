...
dat <- read.csv("data groot/data/data_90min.csv")

items <- names(dat)[grepl("mood_|pat_|phy_|se_", names(dat))]

out <- lapply(1:length(items), readStanResults,
              modelout = "data groot/results/interval hour and half/out/",
              items = items)
...
save(df_pars, df_posteriors, df_bayes_factors, df_pmps,
     file = "data groot/results/interval hour and half/objects.Rdata")
