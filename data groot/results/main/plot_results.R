...
load("data groot/results/main/objects.Rdata")

library(ggplot2)

p_gammas <- plotGammas(df_pars)
ggsave("data groot/results/main/plots/estimated_gammas.pdf", p_gammas,
       width = 12, height = 3)

p_pars <- plotPars(df_pars)
ggsave("data groot/results/main/plots/parameter_uncertainty.pdf", p_pars,
       width = 12, height = 12)

p_phi_gamma <- plotScatterPhiGamma(df_pars)
ggsave("data groot/results/main/plots/estphigamma.pdf", p_phi_gamma,
       width = 4, height = 3)
