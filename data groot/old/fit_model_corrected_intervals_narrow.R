...
fitModel <- function (r, mod, modelout = "data groot/estimation/narrow/", dat, items, seed) {
  out <- tryCatch({
...
modelout <- "data groot/estimation/narrow/"
t_fit <- system.time(
...
readStanResults <- function (r, items) {
  out <- tryCatch({
    out <- readRDS(sprintf("%s%s.rds", "data groot/estimation/narrow/", items[r]))
    out
  }, error = function(e) return(NULL))
...
#   annotate("line", x = seq(-1, 1, 0.001), y = dnorm(seq(-1, 1, 0.001), 0, sqrt(0.1)), linetype = "dotted") +
...
################################################################################
## SAVE OBJECTS
save(res, bftest, pd_long, file = "data groot/estimation/narrow_objects.Rdata")
load("data groot/estimation/narrow_objects.Rdata")
...
ggsave("bayesfactors_narrow.pdf", p, height = 12, width = 12)
...
for(item in items) {
  ...
  saveRDS(
    out,
    file = sprintf("%s%s.rds", "data groot/estimation/narrow/", item)
  )
}
...
readStanResults <- function (r, item, pars) {
  out <- tryCatch({
    out <- readRDS(sprintf("%s%s.rds", "data groot/estimation/narrow/", item[r]))
    out
