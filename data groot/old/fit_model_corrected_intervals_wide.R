...
fitModel <- function (r, mod, modelout = "data groot/estimation/wide/", dat, items, seed) {
  out <- tryCatch({
...
modelout <- "data groot/estimation/wide/"
t_fit <- system.time(
...
readStanResults <- function (r, items) {
  out <- tryCatch({
    out <- readRDS(sprintf("%s%s.rds", "data groot/estimation/wide/", items[r]))
    out
  }, error = function(e) return(NULL))
...
ggplot(pd_long) +
  ...
  annotate("line", x = seq(-1, 1, 0.001), y = dnorm(seq(-1, 1, 0.001), 0, sqrt(2.5)), linetype = "dotted") +
  ...
  geom_segment(y = -Inf, yend = -Inf, x = -1,
               ...
               linewidth = 0.3,
               lineend = "square")
ggsave("bayesfactors_wide.pdf", height = 10, width = 15)
...
for(item in items) {
  ...
  saveRDS(
    out,
    file = sprintf("%s%s.rds", "data groot/estimation/wide/", item)
  )
}
...
readStanResults <- function (r, item, pars) {
  out <- tryCatch({
    out <- readRDS(sprintf("%s%s.rds", "data groot/estimation/wide/", item[r]))
    out
