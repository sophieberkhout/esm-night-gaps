...
readStanResults <- function (r, items, pars) {
  out <- tryCatch({
...
computeBF <- function(r, out, null) {
  out <- tryCatch({
    posterior_samples <- rstan::extract(out[[r]], "gamma")$gamma
    prior_d <- dnorm(null, 0, sqrt(0.5))
    posterior_d <- logspline::dlogspline(null, logspline::logspline(posterior_samples))
...
ggplot(pd_long) +
  facet_wrap(~ .id) +
  annotate("line", x = seq(-1, 1, 0.001), y = dnorm(seq(-1, 1, 0.001), 0, sqrt(0.5)), col = "grey") +
  geom_line(aes(x = x, y = density)) +
  geom_vline(aes(xintercept = null), data = bftest_long_vline, col = rep(c("blue", "green", "red"), 4)) +
  geom_text(aes(x = -1, y = 3, label = text0), data = bftest_long, parse = TRUE, hjust = 0) +
  geom_text(aes(x = -1, y = 2, label = textp), data = bftest_long, parse = TRUE, hjust = 0) +
  geom_text(aes(x = -1, y = 1, label = textc), data = bftest_long, parse = TRUE, hjust = 0)



items <- items[1:4]
res <- lapply(1:length(items), readStanResults, item = items, pars = pars)
BF <- sapply(1:length(items), computeBF, out = res, null = 0)
