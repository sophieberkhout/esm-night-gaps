...
diagnostics <- function (reps, res, out, pars) {
  ...
  prop_ci <- apply(nonzero, 1, .proportion)

  # compute Savage-Dickey density ratio Bayes factors
  log_spline <- lapply(1:reps, fitLogSpline, out)
  bfs <- lapply(1:reps, getBF, res, log_spline)
  
  # check if Bayes factor is larger than 3
  min_bf_3 <- sapply(1:reps, minBF, bfs, min = 3)
  min_bf_5 <- sapply(1:reps, minBF, bfs, min = 5)
  min_bf_10 <- sapply(1:reps, minBF, bfs, min = 10)
  
  prop_bf_3 <- apply(min_bf_3, 1, .proportion)
  prop_bf_5 <- apply(min_bf_5, 1, .proportion)
  prop_bf_10 <- apply(min_bf_10, 1, .proportion)
  
  # check which model has most probability
  # gamma is for pauses model, diff is for stops model
  # and diff_ct is for continues model
  ...
  return(
    data.frame(parameter = names(pars),
               ...
               prop_bf_3 = prop_bf_3,
               prop_bf_5 = prop_bf_5,
               prop_bf_10 = prop_bf_10,
               pref_model = pref_model,
               coverage = coverage_rate, prop_ci = prop_ci)
  )
}
...
getBF <- function(r, res, fit) {
  out <- tryCatch({
    # get density posterior at 0
    posterior_d_0 <- logspline::dlogspline(0, fit[[r]])
    BFsd <- posterior_d_0 / dnorm(0, 0, sqrt(0.5))
    
    phi <- res[[r]][row.names(res[[r]]) == "phi", "median"]
    
    # get density posterior at phi
    posterior_d_phi <- logspline::dlogspline(phi, fit[[r]])
    BFpd <- posterior_d_phi / dnorm(phi, 0, sqrt(0.5))
    
    # get density posterior at phi ^ 7 (continues model)
    posterior_d_phict <- logspline::dlogspline(phi ^ 7, fit[[r]])
    BFcd <- posterior_d_phict / dnorm(phi ^ 7, 0, sqrt(0.5))
    
    data.frame(phi = phi, phict = phi ^ 7,
               BFsd = BFsd, BFpd = BFpd, BFcd = BFcd)
  }, error = function(e) return(NULL))
...
minBF <- function(r, bfs, min) {
  # BFsd = 3 means Hs is three times more likely than Hd
  # but we want to check if Hd is at least min times more likely
  # so we do 1 / BFsd to get BFds
  min_BFsd <- 1 / bfs[[r]]$BFsd > min
  min_BFpd <- 1 / bfs[[r]]$BFpd > min
  min_BFcd <- 1 / bfs[[r]]$BFcd > min
  
  out <- c(NA, NA, min_BFsd, NA, NA, min_BFpd, min_BFcd)
}
...
calcPostModProbs <- function (r, bfs) {
  pmps <- postModelProb(bfs[[r]]$BFpd, bfs[[r]]$BFsd, bfs[[r]]$BFcd)
  # check which of these three models has most probability
  # that model gets 1 the others get 0, so we can compute proportion later
  pref_paus <- which.max(pmps) == 1
  pref_stop <- which.max(pmps) == 2
  pref_cont <- which.max(pmps) == 3
  # pref_diff <- which.max(pmps) == 4 not necessary because adds to 1
...
plotBayesFactors <- function(df, method = "BF_3") {
  ...
  p <- ggplot(df) +
    ...
    geom_line(aes(x = diff + phi, y = prop_bf_3,
                  colour = days, linetype = "BF > 3"), linewidth = 1) +
    geom_point(aes(x = diff + phi, y = prop_bf_3, colour = days),
  ...
  if (method == "CI") {
    p <- p +
      geom_line(aes(x = diff + phi, y = prop_ci,
                    colour = days, linetype = "CI"), linewidth = 1) +
      geom_point(aes(x = diff + phi, y = prop_ci, colour = days),
                 size = 2, stroke = 1.5, fill = "white", shape = 21) +
  ...
  if (method == "BF_10") {
    p <- p +
      geom_line(aes(x = diff + phi, y = prop_bf_10,
                    colour = days, linetype = "BF > 10"), linewidth = 1) +
      geom_point(aes(x = diff + phi, y = prop_bf_10, colour = days),
                 size = 2, stroke = 1.5, fill = "white", shape = 21) +
